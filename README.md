...
**AI-assisted SOC triage that runs entirely on your own infrastructure.**

Security events pasted into Slack are classified by log source, analysed by a
locally hosted LLM using source-specific analyst prompts, and returned as a
structured assessment — with the analyst kept firmly in the decision path.
...
![Slack](https://img.shields.io/badge/Slack-Events%20API-4A154B?logo=slack&logoColor=white)
![Wazuh](https://img.shields.io/badge/Wazuh-log%20source-2296F3)
![License](https://img.shields.io/badge/License-MIT-blue.svg)

[Quick start](#quick-start) ·
[Architecture](#architecture) ·
[Workflow docs](workflows/n8n/ai-soc-assistant/) ·
...
|  |  |
|---|---|
| **Implemented** | 1 n8n workflow · 14 nodes · 7 source-specific analyst prompts |
| **Stack** | n8n + self-hosted Ollama + Slack. Nothing else is deployed. |
| **Log sources** | Wazuh · Windows Security · FortiGate · Palo Alto · F5 BIG-IP ASM · Trend Micro CEF · general Q&A |
| **Trigger** | Slack Events API webhook — analyst-initiated |
| **Output** | Verdict, severity, overall risk, confidence, evidence, recommended actions, missing information |
| **Can it act?** | **No.** There is no node capable of blocking, isolating, disabling or closing anything. |
| **Data egress** | None at inference. The model runs on your hardware. |
...
system prompt written for that source, sends it to a locally hosted Ollama
model, and replies in-thread with a structured analyst-style assessment.
...
> management are **designed and documented but not built** — labelled `Planned`
...
> that is not part of the current stack.
>
> See [Known limitations](#known-limitations) and [Roadmap](#roadmap).

> [!WARNING]
> The n8n Webhook node ships with **no authentication**, and Slack request
> signature verification is **not implemented**. Anyone who learns the URL can
> drive your LLM. Read [Security model](#security-model) before exposing it.
